...
export const Sidebar = ({ activeSection, onSectionChange }: SidebarProps) => {
  ...
  return (
    <>
      ...
      <aside className={`fixed left-0 top-0 h-full bg-card border-r border-border z-50 transition-all duration-300 ${collapsed ? 'w-16' : 'w-72'}`}>
        <div className="flex flex-col h-full">
          <div className="p-4 border-b border-border flex items-center justify-between gap-2">
            {!collapsed && <span className="font-bold text-primary truncate">Selenium Training</span>}
            <button 
      ...
      <div className={`flex-shrink-0 transition-all duration-300 ${collapsed ? 'w-16' : 'w-72'}`} />
    </>
  );
};
